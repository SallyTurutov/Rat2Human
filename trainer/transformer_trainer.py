...
from models.transformer.module.simple_loss_compute import SimpleLossCompute
...
class TransformerTrainer(BaseTrainer):
    ...
    def get_model(self, opt, vocab, device):
        ...
        if opt.pretrain_model and opt.use_pretrained:
            file_name = os.path.join(self.save_path, f'checkpoint/model_pretrained_{opt.num_pretrain_epoch}.pt')
            model = EncoderDecoder.load_from_file(file_name)
            self.LOG.info(f'Loaded pretrained model from: {file_name}')

        # build a model from scratch or load a model from a given epoch
        elif opt.starting_epoch == 1:
            # define model
            model = EncoderDecoder.make_model(vocab_size, vocab_size, N=opt.N, d_model=opt.d_model, d_ff=opt.d_ff,
                                              h=opt.H, dropout=opt.dropout, lambda_attention=opt.lambda_attention,
                                              lambda_distance=opt.lambda_distance,
                                              distance_matrix_kernel=opt.distance_matrix_kernel,
                                              use_encoder_mol_attention=opt.use_encoder_mol_attention,
                                              use_generator_tissue_specification=opt.use_generator_tissue_specification)
        else:
            # Load model
    ...
    def train_epoch(self, dataloader, model, loss_compute, device):
        ...
        for i, batch in enumerate(ul.progress_bar(dataloader, total=len(dataloader))):
            src, trg, src_mask, trg_mask, src_adj_matrix, src_distances_matrix, tissues_list, df = batch
            ...
            out = model(src, src_mask, mode='forward', trg=trg, trg_mask=trg_mask,
                        src_adj_matrix=src_adj_matrix, src_distances_matrix=src_distances_matrix,
        ...
        for i, batch in enumerate(ul.progress_bar(dataloader, total=len(dataloader))):
            ...
            with torch.no_grad():
                ...
                for j in range(trg.size()[0]):
                    ...
                    start_ind = 2  # 2 because 1 for tissue and 1 for (source/ target) organism
    ...
    def train(self, opt):
        ...
        # Data loader
        dataloader_pretrain = self.initialize_dataloader(opt.data_path, opt.batch_size, vocab, 'pretrain')
        ...
        # Pre-Train epoch
        if opt.pretrain_model and not opt.use_pretrained:
            for epoch in range(opt.starting_epoch, opt.starting_epoch + opt.num_pretrain_epoch):
                self.LOG.info("Starting EPOCH #%d", epoch)

                self.LOG.info("Pre-Training start")
                model.train()
                loss_epoch_train = self.train_epoch(dataloader_pretrain, model, SimpleLossCompute(criterion, optim), device)

                self.LOG.info("Pre-Training end")
                self.save(model, optim, f'pretrained_{epoch}', vocab_size, opt)
        ...
        for epoch in range(opt.starting_epoch, opt.starting_epoch + opt.num_epoch):
            ...
            model.train()
            loss_epoch_train = self.train_epoch(dataloader_train, model, SimpleLossCompute(criterion, optim), device)
            ...
            loss_epoch_validation, accuracy, token_accuracy, similarities = self.validation_stat(
                dataloader_validation, model, SimpleLossCompute(criterion, None), device, vocab)

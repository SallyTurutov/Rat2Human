...
def decode(model, src, src_mask, max_len, type, tissues_list, adj_matrix, distances_matrix):
    ys = torch.ones(1)
    ys = ys.repeat(src.shape[0], 1).view(src.shape[0], 1).type_as(src.data)
    # ys shape [batch_size, 1]
    encoder_outputs = model.encode(src, src_mask, adj_matrix, distances_matrix)
    ...
    for _ in range(max_len-1):
        with torch.no_grad():
            ...
            log_prob = model.generator(out[:, -1], tissues_list)

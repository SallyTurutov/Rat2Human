...
class GenerateRunner():
    ...
    def generate(self, opt):
        ...
        for j, batch in enumerate(ul.progress_bar(dataloader_test, total=len(dataloader_test))):
            ...
            src, _, src_mask, _, adj_matrix, distances_matrix, tissues_list, df = batch

            # Move to GPU
            src = src.to(device)
            src_mask = src_mask.to(device)
            smiles = self.sample(model, src, src_mask, tissues_list, adj_matrix, distances_matrix, opt.decode_type,
                                 num_samples=opt.num_samples, max_len=max_len, device=device)
    ...
    def sample(self, model, src, src_mask, tissues_list, adj_matrix, distances_matrix, decode_type, num_samples=10,
               max_len=cfgd.DATA_DEFAULT['max_sequence_length'], device=None):
        ...
        with torch.no_grad():
            while not all(num_valid_batch >= num_valid_batch_desired) and current_trials < max_trials:
                ...
                # sample molecule
                sequences = decode(model, src_current, mask_current, max_len, decode_type, tissues_list, adj_matrix, distances_matrix)
